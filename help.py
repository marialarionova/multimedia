...
if __name__ == "__main__":
    ...
    gw_img = gray_world(img3)
    imsave('gw_img.jpg', gw_img)

    docked_map_gw = docked_map(img3, gw_img, "img_gw-img")

    # 2.3

    sp_gray_img = random_noise(gray_img, mode="s&p", seed=None, clip=True)
    imsave('sp_gray_img.jpg', sp_gray_img)

    # docked_map_sp = docked_map(gray_img, sp_gray_img, "gray-img_sp-gray-img")

    diff_map_sp = diff_map(gray_img, sp_gray_img, "gray-img_sp-gray-img")

    filtered_img = median_filter(sp_gray_img, 3)
    imsave('filtered_img.jpg', filtered_img)
